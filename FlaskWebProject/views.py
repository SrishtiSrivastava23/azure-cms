"""
Routes and views for the Flask application with MSAL integration.
"""
...
def login():
    ...
    # Generate state for MSAL login
    session["state"] = str(uuid.uuid4())
    auth_url = _build_auth_url(scopes=Config.SCOPE, state=session["state"])
    return render_template('login.html', title='Sign In', form=form, auth_url=auth_url)


@app.route(Config.REDIRECT_PATH)  # Absolute URL must match Azure AD redirect URI
def authorized():
    if request.args.get('state') != session.get("state"):
        return redirect(url_for("home"))  # State mismatch
    ...
    if request.args.get('code'):
        ...
        # Log in as admin (or map session["user"] to actual user in DB)
...
def logout():
    ...
    if session.get("user"):  # Logged in via MS
        session.clear()
        return redirect(
...
def _save_cache(cache):
    """Save token cache to session if it changed"""
...
def _build_auth_url(authority=None, scopes=None, state=None):
    """Return the MSAL authorization request URL"""
